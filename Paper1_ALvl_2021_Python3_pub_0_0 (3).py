...
def CheckMoveCommandFormat(Items): # The item is each command that the user gives
  if len(Items) == 3: # Length of items is including the word 
...
def PlayGame(Player1, Player2, Grid):
  ...
  while not (GameOver and Player1Turn): #While the game isnt over and its currently 
    ...
    for C in Commands:
      Items = C.split(" ") #Create a new element for each space
